...
public class q1 implements java.io.Closeable
{
	...
	public final void makecouples() throws IOException 
	{
                ...
                ke++;
                         ...
                         for (j = 0; j < boy_arr.size(); j++)
		{       int temp1 = girl_arr.get(0).attractiveness;
                        int temp2 = 0;
			for (i = 0; i < girl_arr.size(); i++)
			{
				if (girl_arr.get(i+1).attractiveness >= girl_arr.get(i).attractiveness && girl_arr.get(j).commited == false)
				{    temp2 = i+1;
                                      temp1 = girl_arr.get(i+1).attractiveness;
                                }
                        }
                                girl_arr.get(temp2).commited = true;
                                boy_arr.get(j).commited = true;
                                Couples Cp = new Couples(boy_arr.get(j), girl_arr.get(temp2));
